...
def run(protocol: protocol_api.ProtocolContext) -> None:
    ...
    def setup(plate_type: str = "corning_96_wellplate_360ul_flat",
        ...
        """
        tipracks: List[protocol_api.Labware] = [protocol.load_labware('opentrons_96_filtertiprack_1000ul', location=slot) for slot in tiprack_slots]
        ...
        pipette = protocol.load_instrument('p1000_single_gen2', 'right', tip_racks=tipracks)
    ...
    def pick_up_tip(tip_ID: str = None) -> None:
        ...
        """
        global dual_tiprack_state, reduced_tips_info, currently_held_tip
        ...
        if reduced_tips_info is not None:
            if tip_ID not in reduced_tips_info:
                try:
                    ...
                    protocol.comment(f"Using tip {tip_ID_well} in rack 1 for tip ID {tip_ID}.")
                    ...
                    try:
                        ...
                        protocol.comment(f"Using tip {tip_ID_well} in rack 2 for tip ID {tip_ID}.")
            ...
            # Pick up this tip
            tip_rack_idx = 0 if reduced_tips_info[tip_ID][0] == "rack_1" else 1
            protocol.comment(f"Picking up tip {reduced_tips_info[tip_ID]} for tip ID {tip_ID}. Exact arg: {pipette.tip_racks[tip_rack_idx].wells()[reduced_tips_info[tip_ID][1]]}")

            pipette.pick_up_tip(location=pipette.tip_racks[tip_rack_idx].wells()[reduced_tips_info[tip_ID][1]])
            currently_held_tip = tip_ID
            return
    ...
    def return_tip(tip_ID: str = None) -> None:
        ...
        """
        global reduced_tips_info, currently_held_tip
        currently_held_tip = None
        if reduced_tips_info is not None:
    ...
    def change_to_tip(tip_ID: str):
        """
        Changes the current tip to the one specified by tip_ID.
        :param tip_ID: The ID of the tip to change to.
        :raises TiprackEmptyError: If the tip rack is empty.
        """
        global currently_held_tip, active_missile_volume
        if pipette.has_tip:
            if currently_held_tip == tip_ID:
                protocol.comment(f"Already holding tip {tip_ID}.")
                return
            else:
                if currently_held_tip == "missile" and active_missile_volume > 0:
                    protocol.comment(f"Currently holding missile with volume {active_missile_volume}, dumping in source.")
                    # If we are holding a missile, we need to return it to the ammo well
                    pipette.dispense(active_missile_volume, ammo.top())
                    pipette.blow_out(ammo.top())
                    active_missile_volume = 0
                protocol.comment(f"Dropping current tip {currently_held_tip} to pick up new tip {tip_ID}.")
                return_tip(currently_held_tip)
    ...
    def fire_missile(
        ...
        """
        global dual_tiprack_state, reduced_tips_info, active_missile_volume
        ...
        if volume + plate.wells[plate_well].volume > plate.wells[plate_well].max_volume:
            raise WellFullError("Cannot add liquid to well; well is full.")

        # For sterile:
        #pick_up_tip(tip_ID=f"missile_{plate_idx}_{plate_well}")
        # For nonsterile:
        change_to_tip("missile") # dedicated tip for firing missiles

        if active_missile_volume < volume:
            # If we are not holding enough missile liquid, we need to pick up more
            pipette.blow_out(ammo.top()) # blow out any remaining liquid in the pipette, this is the safest way to ensure we don't overfill the tip
            
            pipette.aspirate(pipette.max_volume - 100, ammo.bottom(z=2.5))
            pipette.air_gap(volume=5)
            active_missile_volume = pipette.max_volume - 100 
        ...
        # For nonsterile:
        pipette.dispense(volume, plate.labware[plate_well].top()) # this is above the meniscus to avoid contamination
        active_missile_volume -= volume

        ## Quick mix for sterile:
        #for _ in range(2):
    ...
    def _place_liquid(
        ...
        change_to_tip(f"placement_{liquid_name}")
        remaining = 0
        for well in wells:
            if remaining < default_volume:
                pipette.aspirate(pipette.max_volume-remaining-100, liquid.bottom(z=2.5))
                remaining += pipette.max_volume-remaining-100
    ...
    def end_game() -> None:
        """
        Blows out the missile volume and returns the tip.
        """
        global currently_held_tip
        if currently_held_tip != "missile":
            protocol.comment(f"Not holding a missile tip, but currently holding {currently_held_tip}.")
            return_tip(currently_held_tip)
            return
        
        pipette.blow_out(ammo.top())
        pipette.return_tip(currently_held_tip)

    ### MAIN PROTOCOL ###

    plate_type = "corning_96_wellplate_360ul_flat"
    global dual_tiprack_state, run_flag, reduced_tips_info, currently_held_tip, active_missile_volume
    reduced_tips_info = {}
    ...
    missile_volume = data.get("missile_volume", 100)

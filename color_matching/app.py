...
if make_btn:
    ...
    for slot, vol in volumes.items():
        if vol > 0:
            robot.add_add_color_action(color_well=slot,
...
if clear_btn:
    ...
    try:
        st.session_state.robot.add_refresh_plate_action()
        st.session_state.robot.execute_actions_on_remote()
        st.success("Plate refreshed successfully.")
    except RuntimeError as e:
        st.error(f"Error refreshing plate: {e}")
        st.stop()
...
render_graph = False
for entry in st.session_state.well_data.values():
    recipe = entry["recipe"]
    rgb = entry["rgb"]
...
if new_tiprack_btn:
    try:
        st.session_state.robot.add_refresh_tiprack_action()
        st.session_state.robot.execute_actions_on_remote()
        st.success("Tiprack refreshed successfully.")
    except RuntimeError as e:
        st.error(f"Error refreshing tiprack: {e}")
        st.stop()

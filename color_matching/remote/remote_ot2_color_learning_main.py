...
def run(protocol: protocol_api.ProtocolContext) -> None:
    ...
    def refresh_plate() -> None:
        """
        Resets the plate state to all wells empty.
        """
        protocol.comment("Refreshing plate...")
        for well in plate.wells.values():
            well.volume = 0
        protocol.comment("Plate refreshed.")
        ...
        for action in actions:
            for subaction_name, subaction_args in action.items():
                ...
                if subaction_name == "blink_lights":
                    ...
                    refresh_tiprack()
                elif subaction_name == "refresh_plate":
                    refresh_plate()
